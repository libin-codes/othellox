...
from othellox.game.type import GameState, Player, Square
...
class Othello(App):
    ...
    def update_game(self, coord: Square):
        ...
        # check game over, if over update the heading in header
        if self.game.state.is_over:
            if self.game.state.is_draw:
                self.game_board.update_header(heading="Draw")
            else:
                
                self.game_board.update_header(
                    heading=f"{"⚫" if self.game.state.winner==Player.BLACK else "⚪"} Won!",
                    score=self.game.score
                )
            # disbale the board because the game is over
